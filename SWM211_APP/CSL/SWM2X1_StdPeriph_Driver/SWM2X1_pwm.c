...
*			uint32_t width			发出的触发信号的宽度，取值0、4、8、12、...、252 个PWM计数时钟周期
*			uint32_t trig_chn		触发信号有8个通道，选择此触发信号输出到哪个通道，，取值PWM_TRG_0、PWM_TRG_1、...、PWM_TRG_7
*			uint32_t trig_interval	触发间隔，0 每周期触发   1 间隔1周期触发一次   2 间隔2周期触发一次  ...  7 间隔7周期触发一次
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
void PWM_CmpTrigger(PWM_TypeDef * PWMx, uint16_t match, uint32_t dir, uint32_t width, uint32_t trig_chn, uint32_t trig_interval)
{
	PWMx->CMPTRG = (1         << PWM_CMPTRG_EN_Pos)    |
				   ...
				   /* 在发出指定宽度触发信号的同时，还会在该触发信号宽度的 PWM->CMPTRG.ATP/8 位置处发出adc触发信号，取值0--7 */
...
* 函数名称:	PWM_BrkIntEn()
* 功能说明: 刹车中断使能
* 输    入: uint32_t brkit			brake interrupt type，有效值包括 PWM_BRKIT_BRK0、PWM_BRKIT_BRK1、PWM_BRKIT_BRK2 及其“或”
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
